<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                @else
                    ...
                    @if (session('nom_directeur'))
                        ...
                        <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                            <x-nav-link :href="route('directeur.annee')" :active="request()->routeIs('directeur.annee')">
                                {{ __('Année scolaire') }}
                            </x-nav-link>
                        </div>
                    @else
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div>{{ Auth::user()->name ?? session('nom_directeur') }}</div>
                    ...
                    <x-slot name="content">
                        ...
                        @if (session('id'))
                            <x-dropdown-link :href="route('directeur.logout', session('id'))">
                                {{ __('Déconnexion') }}
                            </x-dropdown-link>
                        @else
                            <!-- Authentication -->
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800">{{ Auth::user()->name ?? session('nom_directeur') }}</div>
                <div class="font-medium text-sm text-gray-500">{{ Auth::user()->email ?? session('id') }}</div>
            </div>
